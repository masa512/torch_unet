...
# This implementation has optional squeeze and excite block after the whole CBR
...
class sqNex(nn.Module):
    ...
    def forward(self,x):
        ...
        #return torch.dot(x ,torch.unsqueeze(torch.unsqueeze(torch.unsqueeze(s,-1),-1),0))
        return x*s
...
class UpDecoder(nn.Module):
    ...
    def __init__(self, in_channels:int, out_channels:int, kernel_size = 3, do_sqNex:bool = False):
        ...
        self.double_conv = DoubleConv(kernel_size = kernel_size, in_channels = in_channels, out_channels=out_channels, do_sqNex = do_sqNex)
